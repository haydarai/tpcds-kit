...
,sret_return_ship_cost sr_return_ship_cost ,sret_refunded_cash sr_refunded_cash ,sret_reversed_charge sr_reversed_charge ,sret_store_credit sr_store_credit ,sret_return_amt+sret_return_tax+sret_return_fee-sret_refunded_cash-sret_reversed_charge-sret_store_credit sr_net_loss FROM s_store_returns
LEFT OUTER JOIN date_dim
ON (cast(sret_return_date as date) = d_date)
LEFT OUTER JOIN time_dim
ON (( cast(substr(sret_return_time,1,2) AS UNSIGNED)*3600
+cast(substr(sret_return_time,4,2) AS UNSIGNED)*60
+cast(substr(sret_return_time,7,2) AS UNSIGNED)) = t_time) LEFT OUTER JOIN item ON (sret_item_id = i_item_id)
...
CREATE VIEW wsv AS
SELECT
d1.d_date_sk ws_sold_date_sk, t_time_sk ws_sold_time_sk, d2.d_date_sk ws_ship_date_sk, i_item_sk ws_item_sk,
...
LEFT OUTER JOIN web_page ON (wlin_web_page_id = wp_web_page_id AND wp_rec_end_date IS NULL) LEFT OUTER JOIN warehouse ON (wlin_warehouse_id = w_warehouse_id)
...
,c1.c_customer_sk wr_refunded_customer_sk ,c1.c_current_cdemo_sk wr_refunded_cdemo_sk ,c1.c_current_hdemo_sk wr_refunded_hdemo_sk ,c1.c_current_addr_sk wr_refunded_addr_sk ,c2.c_customer_sk wr_returning_customer_sk ,c2.c_current_cdemo_sk wr_returning_cdemo_sk ,c2.c_current_hdemo_sk wr_returning_hdemo_sk ,c2.c_current_addr_sk wr_returing_addr_sk
...
LEFT OUTER JOIN time_dim ON ((CAST(SUBSTR(wret_return_time,1,2) AS UNSIGNED)*3600 +CAST(SUBSTR(wret_return_time,4,2) AS UNSIGNED)*60+CAST(SUBSTR(wret_return_time,7,2) AS UNSIGNED))=t_time) LEFT OUTER JOIN item ON (wret_item_id = i_item_id)
...
LEFT OUTER JOIN time_dim ON ((CAST(substr(cret_return_time,1,2) AS UNSIGNED)*3600
+CAST(substr(cret_return_time,4,2) AS UNSIGNED)*60
+CAST(substr(cret_return_time,7,2) AS UNSIGNED)) = t_time)
...
i_item_sk inv_item_sk, w_warehouse_sk inv_warehouse_sk, invn_qty_on_hand inv_quantity_on_hand
FROM s_inventory
